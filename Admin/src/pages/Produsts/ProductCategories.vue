    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h5 class="text-h5 text-white">Categories</h5>
...
const handleSave = async () => {
  try {
    ...
    if (isEditing.value) {
      await axios.put(`${apiUrl}/api/product-categories/${formData.value.id}`, submitData);
      showSnackbar('Category updated successfully!');
    } else {
      await axios.post(`${apiUrl}/api/product-categories`, submitData);
...
const deleteCategory = async () => {
  try {
    ...
    await axios.delete(`${apiUrl}/api/product-categories/${categoryId}`);
    await fetchCategories();
