    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
          <h4 class="text-h5">Categories</h4>
...
const handleSave = async () => {
  try {
    if (!formData.value.name_en || !formData.value.name_lo) {
      showSnackbar('Category name is required in both languages', 'error');
      return;
    }
    ...
    const url = isEditing.value 
      ? `${apiUrl}/api/news-categories/${formData.value.id}`
      : `${apiUrl}/api/news-categories`;
    ...
    showSnackbar(`Category ${isEditing.value ? 'updated' : 'created'} successfully`);
    closeDialog();
  } catch (error) {
...
const deleteCategory = async () => {
  try {
    ...
    await axios.delete(
      `${apiUrl}/api/news-categories/${categoryId}`,
      {
