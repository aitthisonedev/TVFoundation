    ...
    <v-row>
      <v-col cols="12">
        <v-card elevation="2" class="rounded-lg">
          ...
          >
            <h6 class="text-h5 text-white font-weight-bold">Core Values</h6>
            <v-btn
              color="white"
              variant="flat"
      ...
      <v-card class="rounded-lg">
        <v-card-title class="pa-6 bg-primary text-white">
          <span class="text-h5 font-weight-bold"
            >{{ isEditing ? "Edit" : "Add" }} Core Value</span
          >
        </v-card-title>
  ...
  setup() {
    ...
    const fetchContents = async () => {
      try {
        ...
        snackbar.value = {
          visible: true,
          message: error.response?.data?.error || "Error fetching core values",
          color: "error",
        };
      }
    };
    ...
    const submitForm = async () => {
      try {
        ...
        const url = `${apiUrl}/api/core-values${
    ...
    const deleteContent = async () => {
      try {
        await axios.delete(`${apiUrl}/api/core-values/${itemToDelete.value}`);
        await fetchContents();
        snackbar.value = {
          visible: true,
        ...
        snackbar.value = {
          visible: true,
          message: error.response?.data?.error || "Error deleting core value",
          color: "error",
        };
      }
    };
