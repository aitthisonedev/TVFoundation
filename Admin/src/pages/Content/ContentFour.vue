    ...
    <v-col cols="12">
      <v-card elevation="2" class="rounded-lg">
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h6 class="text-h5 text-white font-weight-bold">Who Are We?</h6>
          <v-btn 
            color="white" 
            variant="flat"
          ...
          <v-table hover class="rounded">
            <thead>
              <tr>
                ...
                <th class="text-left font-weight-bold">Video Link</th>
            ...
            <tbody>
              <tr v-for="(content, index) in paginatedContents" :key="content.id">
                ...
                <td class="text-body-2">{{ truncateText(content.title_en, 30) }}</td>
                <td class="text-body-2">{{ truncateText(content.title_lo, 30) }}</td>
                <td class="text-body-2">{{ truncateText(content.video_link, 30) }}</td>
                <td>
                  <v-btn
                    size="small"
                    color="warning"
...
const submitForm = async () => {
  try {
    ...
    if (isEditing.value) {
      await axios.put(`${apiUrl}/api/content-fours/${formData.value.id}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      snackbar.value.message = "Content updated successfully!";
    } else {
      await axios.post(`${apiUrl}/api/content-fours`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
...
const confirmDelete = async () => {
  try {
    ...
    await axios.delete(`${apiUrl}/api/content-fours/${contentId}`);
...
const fetchContents = async () => {
  try {
    const response = await axios.get(`${apiUrl}/api/content-fours`);
