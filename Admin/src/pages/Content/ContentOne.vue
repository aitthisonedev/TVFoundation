    ...
    <v-col cols="12">
      <v-card elevation="2" class="rounded-lg">
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h6 class="text-h5 text-white font-weight-bold">Hello</h6>
          <v-btn 
            color="white" 
            variant="flat"
...
const submitForm = async () => {
  try {
    ...
    if (isEditing.value) {
      await axios.put(`${apiUrl}/api/content-ones/${formData.value.id}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      snackbar.value.message = "Content updated successfully!";
    } else {
      await axios.post(`${apiUrl}/api/content-ones`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
...
const confirmDelete = async () => {
  ...
  try {
    await axios.delete(`${apiUrl}/api/content-ones/${content.id}`);
    await fetchContents();
    snackbar.value = {
      visible: true,
...
const pageCount = computed(() => {
  return Math.ceil(contentList.value.length / itemsPerPage.value);
});
