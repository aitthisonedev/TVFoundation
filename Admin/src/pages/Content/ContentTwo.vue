    ...
    <v-col cols="12">
      <v-card elevation="2" class="rounded-lg">
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h6 class="text-h5 text-white font-weight-bold">About Us</h6>
          <v-btn 
            color="white" 
            variant="flat"
...
const handleImageError = (error) => {
  ...
  error.target.src = '/path-to-fallback-image.jpg'; // Optional: Add a fallback image
};
...
const submitForm = async () => {
  try {
    ...
    if (isEditing.value) {
      await axios.put(`${apiUrl}/api/content-twos/${formData.value.id}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      snackbar.value.message = "Content updated successfully!";
    } else {
      await axios.post(`${apiUrl}/api/content-twos`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
...
const confirmDelete = async () => {
  try {
    ...
    await axios.delete(`${apiUrl}/api/content-twos/${contentId}`);
