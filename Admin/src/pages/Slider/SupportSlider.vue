    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h5 class="text-h5 text-white">Support Sliders</h5>
...
const saveSlider = async () => {
  ...
  try {
    ...
    await axios.post(`${apiUrl}/api/sliders-support`, formData, {
...
const confirmDelete = async () => {
  ...
  const slider = sliderList.value[index];
  try {
    await axios.delete(`${apiUrl}/api/sliders-support/${slider.id}`);
