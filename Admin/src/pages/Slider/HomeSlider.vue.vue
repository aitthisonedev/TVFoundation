    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h4 class="text-h5">Slider Home</h4>
...
const fetchSliders = async () => {
  try {
    const response = await axios.get(`${apiUrl}/api/sliders-home`);
...
const saveSlider = async () => {
  ...
  try {
    ...
    await axios.post(`${apiUrl}/api/sliders-home`, formData, {
...
const confirmDelete = async () => {
  ...
  const slider = sliderList.value[index];
  try {
    await axios.delete(`${apiUrl}/api/sliders-home/${slider.id}`);
