    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center pa-6 bg-primary">
          <h5 class="text-h5 text-white">Slider project</h5>
...
const fetchSliders = async () => {
  try {
    const response = await axios.get(`${apiUrl}/api/sliders-project`);
...
const saveSlider = async () => {
  ...
  try {
    ...
    await axios.post(`${apiUrl}/api/sliders-project`, formData, {
...
const confirmDelete = async () => {
  ...
  const slider = sliderList.value[index];
  try {
    await axios.delete(`${apiUrl}/api/sliders-project/${slider.id}`);
