const SliderProject = require('../models/SliderProjectModel');
...
exports.getAllSliders = async (req, res) => {
  try {
    const sliders = await SliderProject.findAll();
...
exports.getSliderById = async (req, res) => {
  try {
    const { id } = req.params;
    const slider = await SliderProject.findOne({ where: { id } });
  ...
  return [
    ...
    async (req, res) => {
      try {
        ...
        let slider;
        if (id) {
          slider = await SliderProject.findByPk(id);
          if (slider) {
            if (image) {
              slider.image = image;
            }
          ...
          slider = await SliderProject.create({
            title_en,
            title_lo,
            description_en,
            description_lo,
...
exports.deleteSlider = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await SliderProject.destroy({ where: { id } });
    if (deleted) {
