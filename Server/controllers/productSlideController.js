const ProductSlide = require('../models/productSlideModel');
const createImageUpload = require('../configs/multerConfig');
const path = require('path');

exports.getAllSlides = async (req, res) => {
  try {
    const slides = await ProductSlide.findAll();
  ...
  return [
    ...
    async (req, res) => {
      try {
        ...
        const slide = await ProductSlide.create({ 
          name_en, 
          name_lo,
          image 
        });
...
exports.updateSlide = (folder) => {
  ...
  return [
    ...
    async (req, res) => {
      try {
        const { id } = req.params;
        const slide = await ProductSlide.findByPk(id);
