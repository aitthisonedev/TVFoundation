const SliderHome = require('../models/SliderHomeModel');
...
exports.getSliderById = async (req, res) => {
  try {
    const { id } = req.params;
    const slider = await SliderHome.findOne({ where: { id } });
  ...
  return [
    ...
    async (req, res) => {
      try {
        ...
        let slider;
        if (id) {
          slider = await SliderHome.findByPk(id);
          if (slider) {
            if (image) {
              slider.image = image;
            }
          ...
          slider = await SliderHome.create({
            title_en,
            title_lo,
            description_en,
            description_lo,
...
exports.deleteSlider = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await SliderHome.destroy({ where: { id } });
    if (deleted) {
