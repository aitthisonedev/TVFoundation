const SliderSupport = require('../models/SliderSupportModel');
...
exports.getAllSliders = async (req, res) => {
  try {
    const sliders = await SliderSupport.findAll();
...
exports.getSliderById = async (req, res) => {
  try {
    const { id } = req.params;
    const slider = await SliderSupport.findOne({ where: { id } });
  ...
  return [
    ...
    async (req, res) => {
      try {
        ...
        let slider;
        if (id) {
          slider = await SliderSupport.findByPk(id);
          if (slider) {
            if (image) {
              slider.image = image;
            }
          ...
          slider = await SliderSupport.create({
            title_en,
            title_lo,
            description_en,
            description_lo,
...
exports.deleteSlider = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await SliderSupport.destroy({ where: { id } });
    if (deleted) {
